"""
Script mejorado para reemplazar correctamente la función download_document_pdf
"""
...
nueva_funcion_lines = '''@login_required
...
# Encontrar inicio de download_document_pdf
inicio = None
for i, line in enumerate(lines):
    if '@login_required' in line and i+1 < len(lines) and 'def download_document_pdf' in lines[i+1]:
        inicio = i
        break

# Encontrar verify_document (fin de download_document_pdf)
fin = None
for i, line in enumerate(lines):
    if 'def verify_document(request, hash):' in line:
        fin = i
        break

if inicio is None or fin is None:
    print(f"❌ Error: No se encontraron las funciones")
    print(f"   inicio: {inicio}")
    print(f"   fin: {fin}")
    exit(1)

# Crear nuevo archivo
nuevas_lines = lines[:inicio] + [l + '\n' for l in nueva_funcion_lines] + lines[fin:]

# Guardar
with open('censoapp/document_views.py', 'w', encoding='utf-8') as f:
    f.writelines(nuevas_lines)

print(f"✅ Función reemplazada correctamente")
print(f"   - Línea inicio: {inicio + 1}")
print(f"   - Línea fin: {fin + 1}")
print(f"   - Líneas eliminadas: {fin - inicio}")
print(f"   - Líneas nuevas: {len(nueva_funcion_lines)}")
